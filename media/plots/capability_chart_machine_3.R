...
3
machine_data <- filtered_data %>% filter(Machine == 3)
p_capability <- ggplot(machine_data, aes(x = PartLength)) +
  ...
  labs(
    title = "Process Capability for PartLength (Machine 3 at 338K, 200kPa)",
    x = "PartLength",
    y = "Density"
  ) +
  theme_minimal() +
...
saveWidget(plotly_capability, "/content/project/media/plots/capability_chart_machine_3.html", selfcontained = TRUE)
