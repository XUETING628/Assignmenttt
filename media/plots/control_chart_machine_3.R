...
3
machine_data <- filtered_data %>% filter(Machine == 3) %>% mutate(index = row_number())
...
p_control <- ggplot(machine_data, aes(x = index, y = PartLength)) +
  ...
  labs(
    title = "Control Chart for PartLength (Machine 3 at 338K, 200kPa)",
    x = "Observation Index",
    y = "PartLength"
  ) +
  theme_minimal() +
...
saveWidget(plotly_control, "/content/project/media/plots/control_chart_machine_3.html", selfcontained = TRUE)
